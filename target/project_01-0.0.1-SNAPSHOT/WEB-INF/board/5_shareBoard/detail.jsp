...
<script src="${contextPath}/resources/js/reply/sharereply.js"></script>
<script src="${contextPath}/resources/js/board/detail/shareboarddetail.js"></script>
...
<div class="container">
	...
	<table class="table">
		...
		<tr>
			...
			<th>상태</th>
			<td>
				<c:if test="${board.isend eq '진행중'}">
						<c:if test="${auth.id==board.writer or auth.grade eq '관리자' or  auth.grade eq '운영자'}">
							${board.isend}
							<form action="${contextPath}/board/shareboard/isEnd">
								<a type="submit" class="btn btn-danger" href="${contextPath}/board/shareboard/isEnd?bno=${board.bno}">종료하기 </a>
							</form>
							/ 추천 : ${board.suggest}
						</c:if>
						<c:if test="${auth.id != board.writer and not empty auth}">
							${board.isend} / 
							<a type="button" class="btn btn-warning suggest" href="${contextPath}/board/shareboard/suggest?bno=${board.bno}">
							추천 : ${board.suggest}</a>(누르면 추천됩니다.)
						</c:if>
						<c:if test="${empty auth.id}">
							${board.isend} / 추천 : ${board.suggest}회
						</c:if>						
					</c:if>
					<c:if test="${board.isend eq '종료'}">
						${board.isend} / 추천 : ${board.suggest}회
					</c:if>
			</td>
		</tr>
		<tr>
			<th>제목</th>
			<td colspan="3">
				<input type="text" name="title" class="form-control" value="[${board.isend}] ${board.title}" readonly="readonly">
			</td>
		</tr>
		<tr>
			<th>내용</th>
			...
			<td colspan="3">
				...
				<div class="my-3">
					<c:if test="${not empty board.imageFile}">
						...
						<div class="preview">
							<img class="origImg" src="${contextPath}/fileDownload?no=${board.bno}&imageFileName=${board.imageFile}&path=board/shareboard">
						</div>
					</c:if>
		...
		<tr>
			<td colspan="4" class="text-center">
				<c:if test="${auth.id eq board.writer or auth.grade eq '관리자'}">
					...
					<c:if test="${board.isend eq false}">
						<button type="button" class="btn btn-danger Ended">종료</button>
					</c:if>
					...
					<th colspan="2" >
						<ul class="d-flex justify-content-between">
							...
							<li class="form-inline">작성자 :
								<input type="text" class="reply_writer form-control ml-2" value="${auth.nickname}" readonly="readonly">
								<input type="text" class="reply_writernick" value="${auth.id}" readonly="readonly" hidden>
							</li>
						</ul>
					</th>
				</tr>
