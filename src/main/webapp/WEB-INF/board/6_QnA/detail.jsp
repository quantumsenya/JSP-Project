...
<script src="${contextPath}/resources/js/reply/qnareply.js"></script>
<script src="${contextPath}/resources/js/board/detail/qnadetail.js"></script>
			...
			<td>
				<c:if test="${board.isend eq '진행중'}">
					<c:if test="${auth.id==board.writer or auth.grade eq '관리자' or  auth.grade eq '운영자'}">
						${board.isend}
						<form action="${contextPath}/board/qna/isEnd">
							<a type="submit" class="btn btn-danger" href="${contextPath}/board/qna/isEnd?bno=${board.bno}">종료하기</a>
						</form>
					</c:if>
			...
			<td colspan="3">
				...
				<div class="my-3">
					<c:if test="${not empty board.imageFile}">
						...
						<div class="preview">
							<img class="origImg" src="${contextPath}/fileDownload?no=${board.bno}&imageFileName=${board.imageFile}&path=board/qna">
						</div>
					</c:if>
