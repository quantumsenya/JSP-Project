let replyService = {
	...
	list : function(bno){
		...
		$.ajax({
			type : 'get',
			url : `${contextPath}/reply/areaReplyList`,
			data : {bno : bno},
	...
	write : function(replyVO){
		$.ajax({
			type : 'post',
			url : `${contextPath}/reply/areaReplyWrite`,
			data : replyVO,
	...
	remove : function(replyVO){ // 댓글 삭제
		$.ajax({
			type : 'post',
			url : `${contextPath}/reply/areaReplyRemove`,
			data : replyVO,
			success : function(result){
				$('#feedback').find('.modal-body').html(result);
				$('#feedback').modal('show');
		...
		for(let r of replyList){
			output+= 
				`<li class="list-group-item d-flex justify-content-between bg-light text-dark">
					<div>
						<p>${r.reply}</p>
						<span class="badge badge-pill badge-primary">${r.writernick}(${r.writer})</span>
