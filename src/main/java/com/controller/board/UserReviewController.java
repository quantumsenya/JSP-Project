...
import com.service.boardServices.UserReviewService;

@WebServlet("/board/userreview/*")
public class UserReviewController extends HttpServlet {
	...
	public void init() throws ServletException {
		...
		multiReq = new FileUpload("board/userreview/");
	}
	...
	protected void doHandle(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		final String PREFIX = "/WEB-INF/board/3_userReview/";
		...
		if(pathInfo==null || pathInfo.equals("/") ||pathInfo.equals("/list")) {
			List<BoardVO> boardList = service.userReviewList();
		...
		// 글 상세
			...
			service.upUserReviewViews(bno);
			BoardVO board = service.selectUserReview(bno);
		...
		// 추천하기
			...
			String paramBno = request.getParameter("bno");
			int bno = Integer.parseInt(paramBno);
			service.upUserReviewSuggest(bno);
			response.sendRedirect(contextPath+"/board/userreview/detail?bno="+bno);
			return;
		}
		...
		// 글쓰기 처리
			...
			BoardVO vo = BoardVO.builder()
					...
					.build();
			int boardNo = service.addUserReview(vo);
			if(imageFile!=null && imageFile.length()>0) {
				multiReq.uploadImage(boardNo, imageFile);
			}
			response.sendRedirect(contextPath+"/board/userreview/");
			return;
		}
		
		
		// 글수정 처리
		else if (pathInfo.equals("/modUserReview")) {
			Map<String, String> req = multiReq.getMultipartRequest(request);
		...
		// 글삭제
		else if (pathInfo.equals("/removeUserReview")) {
			Map<String, String> req = multiReq.getMultipartRequest(request);
			String paramBno = req.get("bno");
			int bno = Integer.parseInt(paramBno);
			service.removeUserReview(bno);
